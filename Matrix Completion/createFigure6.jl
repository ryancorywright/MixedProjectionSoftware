...
results_template = DataFrame(n=Int[], r=Int[], p=Real[], gamma=Real[], err_OA_inout=Real[], rnk_OA_inout=Int[], solve_OA_inout=Real[], nc_OA_inout=Int[], gap_OA_inout=Real[], cuts_OA_inout=Int[], nc_OA_inout_total=Int[], theSeed=Int[])
...
for ARG in ["1"]
    for iter in 1:1
        ...
        r=1
        seeds=1:20
        gammaMults=[0.1, 0.316227766, 1.0,3.16227766,  10.0,31.6227766, 100.0,316.227766, 1000.0,3162.27766, 10000.0,31622.7766, 100000.0,316227.766,  1000000.0,3162277.66016837, 10000000.0,31622776.6016837, 100000000.0, 316227766.016837] # You will need to alter the gamma values in here to generate the full range of points shown
        ...
        t_OA_inout=@elapsed X_OA, nc_OA_inout, cuts_OA_inout, gap_OA_inout, nc_agg=getMatrixFrobeniusNorm_cuttingplanesmultitree_exact_inout(coords, entries, M, n, numEntriesSampled, k, gamma, Y_relax, X_AM, true, "_n_"*string(n)*"p_"*string(p)*"ID_"*string(array_num))
        # Test solution on overall data.
        u,v,=svd(X_OA)
        ...
        push!(results_exact, [n, r, p, gamma, err_OA_inout, rnk_OA_inout, t_OA_inout, nc_OA_inout, gap_OA_inout, cuts_OA_inout, nc_agg, theSeed])
        CSV.write("sensitivity_analysis_multitree_n_"*string(n)*"_p_"*string(p)*".csv", results_exact, append=true)
        end
    end
